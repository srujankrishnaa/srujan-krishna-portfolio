...
const CpuArchitecture = ({
  ...
  const lineColor = isDark ? "#d8bd1066" : "currentColor";
  const cpuFill = isDark ? "#161b22" : "#181818";
  const markerFill = isDark ? "#d8bd10" : "black";
  const markerStroke = isDark ? "#d8bd1088" : "#232323";
  const connGradStart = isDark ? "#d8bd1088" : "#4F4F4F";
  const connGradEnd = isDark ? "#d8bd1033" : "#121214";

  return (
    <svg
      className={cn(isDark ? "text-[#d8bd1044]" : "text-muted", className)}
      width={width}
      height={height}
      viewBox="0 0 200 100"
    >
      ...
      <g>
        ...
        <rect x="80" y="40" width="40" height="20" rx="2" fill={cpuFill} filter="url(#cpu-light-shadow)" />
        <text
          x="100" y="52.5" fontSize="3.8" textAnchor="middle"
      ...
      <defs>
        ...
        <marker id="cpu-circle-marker" viewBox="0 0 10 10" refX="5" refY="5" markerWidth={lineMarkerSize} markerHeight={lineMarkerSize}>
          <circle id="innerMarkerCircle" cx="5" cy="5" r="2" fill={markerFill} stroke={markerStroke} strokeWidth="0.5">
        ...
        <linearGradient id="cpu-connection-gradient" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={connGradStart} />
          <stop offset="60%" stopColor={connGradEnd} />
        </linearGradient>
